...
class BasicBlockHeartNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            # 确保下采样后的尺寸与主路径的尺寸匹配
            identity = F.max_pool1d(identity, self.stride) if identity.size(-1) > out.size(-1) else identity

            # 确保两个张量在所有维度上的尺寸都匹配
        if identity.size(-1) != out.size(-1):
            print(f"Identity size: {identity.size()}, Out size: {out.size()}")
            raise ValueError("Identity and output sizes do not match")
...
class HeartNet(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(p=0.6)  # 定义Dropout层，丢弃概率为0.5
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        for _ in range(1, blocks+2):       # 这里增加了每层的block数量，比如增加2个  for _ in range(1, blocks+2)
            layers.append(
                block(
                    self.inplanes,
                    planes,
...
#model = HeartNet().to(device=device)
#module = model.conv1
